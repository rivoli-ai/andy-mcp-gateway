...
public class McpSubPathAwareAuthenticationHandler : AuthenticationHandler<McpAuthenticationOptions>, IAuthenticationRequestHandler
{
    ...
    private async Task<bool> HandleResourceMetadataRequestAsync()
    {
        ...
        {
            var resourceMetadataUri = Options.ResourceMetadataUri
                ?? throw new InvalidOperationException("ResourceMetadataUri must be set when OnResourceMetadataRequest is configured.");
    ...
    internal static ProtectedResourceMetadata? CloneResourceMetadata(ProtectedResourceMetadata? resourceMetadata, string subPath)
    {
        ...
        var clone = resourceMetadata.Clone();
        clone.Resource = CombineResourceWithSubPath(resourceMetadata.Resource, subPath);
        return clone;
    }

    private static string? CombineResourceWithSubPath(string? baseResource, string subPath)
    {
        if (baseResource is null)
        {
            return null;
        }
        ...
        if (Uri.TryCreate(baseResource, UriKind.Absolute, out var baseUri))
        {
            return new Uri(baseUri, subPath).AbsoluteUri;
        }

        var trimmedBase = baseResource.TrimEnd('/');
        var path = subPath.StartsWith('/') ? subPath : "/" + subPath;
