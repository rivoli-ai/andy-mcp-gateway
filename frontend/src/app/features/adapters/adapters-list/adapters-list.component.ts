...
export class AdaptersListComponent implements OnInit {
  ...
  /** Grid cards: mirror unhealthy corner — green fold when last health is OK (not disabled). */
  isAdapterHealthy(adapter: McpAdapter): boolean {
    if (this.isAdapterUnhealthy(adapter)) {
      return false;
    }
    const s = adapter.status;
    if (typeof s === 'string' && s.toLowerCase() === 'disabled') {
      return false;
    }
    if (typeof s === 'number' && s === AdapterStatus.Disabled) {
      return false;
    }
    if (adapter.isHealthy === true) {
      return true;
    }
    if (typeof s === 'string' && s.toLowerCase() === 'healthy') {
      return true;
    }
    if (typeof s === 'number' && s === AdapterStatus.Healthy) {
      return true;
    }
    return false;
  }
